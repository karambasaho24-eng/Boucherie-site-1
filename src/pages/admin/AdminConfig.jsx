...
export default function AdminConfig() {
  ...
  function handleChange(e) {
    ...
    setConfig((c) => ({ ...c, [name]: type === 'number' ? Number(value) : value }))
  }
  ...
  async function handleSave() {
    ...
    try {
      await updateSiteConfig({
        ...
        edit_deadline_minutes: config.edit_deadline_minutes ?? 60,
      })
      setSuccess(true)
    ...
    <div className="admin-section">
      ...
      <div className="config-grid">
        ...
        <div className="config-block">
          ...
          {/* Pas de capture="environment" : le client choisit entre appareil photo et galerie */}
        ...
        <div className="config-block config-block-highlight">
          <h4>Délai de retrait</h4>
          <div className="field">
            <label>Délai par défaut appliqué à chaque commande</label>
            <input
              className="input"
              name="pickup_delay"
              value={config.pickup_delay || ''}
              onChange={handleChange}
              placeholder="ex : avant 19h, 2h après confirmation, jusqu'à la fermeture"
            />
            <p className="field-hint">
              Ce texte sera affiché sur le ticket de caisse du client et dans la liste admin.
              Si vous indiquez un format numérique (ex&nbsp;: «&nbsp;2h&nbsp;»), les commandes non retirées
              passeront automatiquement en statut <strong>Expirée</strong> dans l'interface admin.
            </p>
          </div>
        </div>

        <div className="config-block config-block-highlight">
          <h4>Délai de modification client</h4>
          <div className="field">
            <label>Durée pendant laquelle le client peut encore demander une modification (en minutes)</label>
            <input
              className="input"
              name="edit_deadline_minutes"
              type="number"
              min={0}
              value={config.edit_deadline_minutes ?? 60}
              onChange={handleChange}
              placeholder="60"
            />
            <p className="field-hint">
              Après ce délai (calculé depuis la création de la commande), le message affiché au client lui indiquera
              de vous appeler pour toute modification. Saisir 0 pour désactiver le message.
            </p>
          </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        .config-block-highlight { background: var(--color-paper-dim); }
        .config-block h4 {
